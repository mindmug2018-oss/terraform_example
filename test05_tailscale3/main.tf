...
# Public Subnet (NAT Instance ec2가 위치할 곳)
...
resource "aws_route" "to_onpremise_public" {
  route_table_id         = aws_route_table.public_rt.id
  destination_cidr_block = "192.168.61.0/24"
  network_interface_id   = aws_instance.my_ec2.primary_network_interface_id
}

# Private Subnet에서 172.16.8.0/24로 갈 때 EC2를 거치도록 설정
...
# [수정] Public IP 출력 제거 및 Private IP만 유지
