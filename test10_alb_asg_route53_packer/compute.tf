# terraform_example/test10_alb_asg_route53_packer/compute.tf
...
# 3. 직접 만든 최신 ami 를 가져오는 필터 설정
data "aws_ami" "latest_web" {
  most_recent = true
  # 직접만든 AMI 의 owner 는 "self"
  owners      = ["self"]
  filter {
    name   = "name"
    # packer 에서 만든 이름 참조
...
resource "aws_launch_template" "lt" {
  ...
  # 직접 만든 ami 선택 (추후에 직접 만든 ami 를 선택할 수 있음)
  ...
  # ami 혹은 user_data 가 변경이 되었을 때 default 버전을 변경하도록 한다
...
resource "aws_autoscaling_group" "asg" {
  ...
  launch_template {
    ...
    # launch template 의 소스코드가 바뀌어서 버전 번호가 올라가면 바로 감지할 수 있도록 한다
